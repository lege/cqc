...
function dmeng_comment_mail_notify($comment_id, $comment_object){
	...
	if( $comment_object->user_id != $post->post_author ){
		$send_email[] = array(
			...
			'title' => sprintf( __('%1$s在%2$s中回复你','momo'), $comment_object->comment_author, $post->post_title ),
			'headline' => sprintf( __('%1$s，你好！','momo'), get_the_author_meta('display_name', $post->post_author) ),
			'content' => sprintf( __('%1$s在文章<a href="%2$s" target="_blank">%3$s</a>中回复你了，快去看看吧：<br> %4$s','momo'), $comment_object->comment_author, $comment_link, $post->post_title, $comment_text )
		);
	}
	...
	if( $comment_object->comment_parent ) {
		...
		if( $comment_author_email != $comment_parent_email && $comment_parent_email != $post_author_email ){
			$send_email[] = array(
				...
				'title' => sprintf( __('%1$s在%2$s中回复你','momo'), $comment_object->comment_author, $post->post_title ),
				'headline' => sprintf( __('%1$s，你好！','momo'), ( $comment_parent->user_id ? get_the_author_meta('display_name', $comment_parent->user_id) : $comment_parent->comment_author ) ),
				'content' => sprintf( __('%1$s在文章<a href="%2$s" target="_blank">%3$s</a>中回复你了，快去看看吧：<br> %4$s','momo'), $comment_object->comment_author, $comment_link, $post->post_title, $comment_text )
			);
		}
	}
	...
	if( $post_author_email != $admin_email ){
		$send_email[] = array(
			'address' => $admin_email,
			'uid' => 0,
			'title' => sprintf( __('%1$s上的文章有了新的回复','momo'), get_bloginfo('name') ),
			'headline' => sprintf( __('%1$s管理员，你好！','momo'), get_bloginfo('name') ),
			'content' => sprintf( __('%1$s回复了文章<a href="%2$s" target="_blank">%3$s</a>，快去看看吧：<br> %4$s','momo'), $comment_object->comment_author, $comment_link, $post->post_title, $comment_text )
		);
	}
...
function dmeng_retrieve_password_message($message, $key){
	...
	$message = '<h3>'.sprintf( __('%1$s，你好！','momo'), $user_data->display_name ).'</h3>';
	$message .= '<p>';
	$message .= sprintf( __('有人要求重设您在%1$s的帐号密码（用户名：%2$s）。若这不是您本人要求的，请忽略本邮件，一切如常。 要重置您的密码，请打开下面的链接：%3$s','momo'),
												get_bloginfo('name'),
												$user_login,
...
function dmeng_email_verify(){
	...
	if( is_email($_POST['email']) ){
		...
		if( $exists_id && $exists_id != $user_id ){
			die( sprintf(__('这个电子邮件地址（%s）已经被使用，请换一个。','momo'),  $_POST['email']) );
		}

		$verify_email = $user_info->dmeng_verify_email;
		
		if( $verify_email==$_POST['email'] ){
			die( sprintf(__('这个电子邮件地址（%s）你已经验证，无须再次验证。','momo'),  $_POST['email']) );
		}
		
		$transient_key = 'dmeng_email_verify_'.$user_id;
		$transient = (array)json_decode(get_transient( $transient_key ));
		if ( $transient ) {
			die( sprintf(__('每五分钟只能申请一次，你在 %1$s 申请了验证 %2$s ，请验证这个邮箱或稍后再试。','momo'),  $transient['time'], $transient['email'] ) );
		}
		...
		dmeng_send_email(
			$_POST['email'],
			sprintf(__('%1$s用户邮箱验证', 'momo'), get_bloginfo('name') ),
			'<h3>'.sprintf(__('%1$s，你好！', 'momo'), $user_info->display_name ).'</h3>'.
			'<p>'.sprintf(__('请点击下面的链接来验证你的邮箱地址（本链接从 %1$s 开始五分钟内有效）', 'momo'), $current_time ).'</p>'.
			'<p><a href="'.$verify_url.'" target="_blank">'.$verify_url.'</a></p>'.
			'<p>'.__('如果通过点击以上链接无法访问，请将该网址复制并粘贴至新的浏览器窗口中', 'momo').'</p>'
		);
		...
		die( sprintf(__('验证邮件已发送到（%s）邮箱，请尽快验证（五分钟内有效）。','momo'),  $_POST['email']) );
		
	}else{
		die( __('请输入一个有效的电子邮件地址！！！', 'momo') );
	}
...
function dmeng_email_verify_template(){
	if( ( is_home() || is_front_page() ) && isset($_GET['action']) && isset($_GET['ehash']) && isset($_GET['_wp_nonce']) ){
		if( $_GET['action']=='email_verify' ){
			...
			$wp_title =  __('邮箱认证', 'momo');
			
			$user_id = get_current_user_id();
			
			if( $user_id==0 ){
				wp_die( sprintf( __('要验证邮箱，请先<a href="%1$s">登录</a>，已在别的页面登录<a href="%2$s">点击刷新</a>即可。', 'momo'), wp_login_url( dmeng_get_current_page_url() ), 'javascript:location.reload()'), $wp_title , array( 'response'=>503 ));
			}
			
			if( dmeng_email_verify_nonce($_GET['ehash']) != $_GET['_wp_nonce'] ){
				wp_die( sprintf( __('咦？你肯定这是你的验证链接吗？<br><br>安全认证码无效，请到<a href="%1$s">个人资料页</a>重新获取链接。', 'momo'), dmeng_get_user_url('profile') ), $wp_title , array( 'response'=>503 ));
			}
			...
			if( $transient && isset($transient['email']) ){
				...
				if( wp_hash($transient['email'])==$_GET['ehash'] ){
					...
					if ( is_wp_error( $user_id ) ) {
						...
						wp_die( sprintf(__('邮箱信息更新出错，请重试！<br><br><a href="%1$s">&laquo; 返回个人资料页</a>', 'momo'), dmeng_get_user_url('profile')) , $wp_title , array( 'response'=>503 ));
						
					} else {
						
						update_user_meta($user_id, 'dmeng_verify_email', $transient['email']);
						delete_transient( $transient_key );
						wp_die( sprintf(__('恭喜！邮箱地址（%1$s）验证成功<br><br><a href="%2$s">&laquo; 返回个人资料页</a>', 'momo') , $transient['email'], dmeng_get_user_url('profile')), $wp_title , array( 'response'=>503 ));
			...
			wp_die( sprintf(__('验证链接无效<br><br><a href="%1$s">&laquo; 返回个人资料页</a>', 'momo'), dmeng_get_user_url('profile')) , $wp_title , array( 'response'=>503 ));
		}
	}
}
...
function dmeng_verify_email_methods($profile_fields) {
	...
	$profile_fields['dmeng_verify_email'] = sprintf(__('已验证邮箱 (<a href="%s" target="_blank">?</a>)', 'momo'), dmeng_get_user_url('profile').'#pass');
