...
function dmeng_gift_init(){
    register_post_type( 'gift', array(
      ...
      'labels'  => array(
		'name'               => __('积分换礼', 'momo'),
		'singular_name'      => __('积分换礼', 'momo'),
		'menu_name'          => __('积分换礼', 'momo'),
		'name_admin_bar'     => __('积分换礼', 'momo'),
		'add_new'            => __('添加礼品', 'momo'),
		'add_new_item'       => __('添加礼品', 'momo'),
		'new_item'           => __('新礼品', 'momo'),
		'edit_item'          => __('编辑礼品', 'momo'),
		'view_item'          => __('查看礼品', 'momo'),
		'all_items'          => __('全部礼品', 'momo'),
		'search_items'       => __('搜索礼品', 'momo'),
		'parent_item_colon'  => __('礼品上级：', 'momo'),
		'not_found'          => __('没有找到礼品', 'momo'),
		'not_found_in_trash' => __('回收站里没有礼品', 'momo'),
	)
    ) );
    //~ 礼品页重写规则
    add_rewrite_rule(
...
function dmeng_register_gift_meta_box(){
	...
	foreach( array( 'info'=>__('礼品信息', 'momo'), 'content'=>__('礼品内容', 'momo') ) as $gift_key=>$gift_title ){
		add_meta_box(
	...
	function dmeng_gift_meta_callback_info($post){
		...
		$output .= sprintf( '<p>%s</p>', __('填写信息只能是数字，文字会被转为 0', 'momo') );
		...
		$output .= '<p><label for="dmeng_gift_express">'.__('物流', 'momo').'</label><select name="dmeng_gift_express" id="dmeng_gift_express">';
	...
	function dmeng_gift_meta_callback_content($post){
		...
		echo '<p>'.__('以下内容在用户使用积分兑换后可见，礼品介绍请放在文章内容中。注意：编辑后需要保存文章才可保存', 'momo').'</p>';
		wp_editor( stripslashes(htmlspecialchars_decode($gift->content)), 'dmeng_gift_content', array( 'media_buttons' => false ) );
		
		$output = '<p><a class="button select_gift_attachment" href="javascript:;" data-uploader_title="'.__('选择附件', 'momo').'" data-uploader_button_text="'.__('添加附件', 'momo').'">'.__('添加附件', 'momo').'</a></p>';
		$output .= '<p>'.__('附件地址会被加密，请不要泄露真实地址！名称是附件的标题，所有人都可见。注意：本功能只适合小文件传输。', 'momo').'</p>';
		$output .= '<ul id="dmeng_gift_attachment">';
		if($gift->attachment && is_array($gift->attachment)){
			foreach($gift->attachment as $gift_attachment){
				$output .= '<li><a href="javascript:;" class="delete">'.__('删除', 'momo').'</a><input type="hidden" name="dmeng_gift_attachment[]" value="'.$gift_attachment.'"><label>ID</label>'.$gift_attachment.'<label>'.__('名称', 'momo').'</label><a href="'.get_edit_post_link($gift_attachment).'" target="_blank">'.get_the_title($gift_attachment).'</a></li>';
			}
		}
		$output .= '</ul>';
		echo $output;
...
jQuery('.select_gift_attachment').live('click', function( event ){
    ...
    file_frame.on( 'select', function() {
		...
		jQuery('#dmeng_gift_attachment').append('<li><a href="javascript:;" class="delete"><?php _e('删除', 'momo');?></a><input type="hidden" name="dmeng_gift_attachment[]" value="'+attachment.id+'"><label>ID</label>'+attachment.id+'<label><?php _e('名称', 'momo');?></label><a href="'+attachment.editLink+'" target="_blank">'+attachment.title+'</a></li>');
    });
...
function dmeng_exchange_ajax_callback(){
	...
	if($stock!=0 && $stock<= count($buyers)){
		...
		if( $max!=0 && $max<= $exchange_num){
			...
			$data['error'] = __('你已达到最大兑换数，不能再兑换！', 'momo');
			...
			if($gift_credit > $credit){
				...
				if( $gift_credit > 0 ) dmeng_credit_to_void( $uid , $gift_credit, sprintf(__('花费了%1$s积分兑换%2$s','momo') ,$gift_credit, $gift_title) );
				dmeng_exchange( $pid, $uid );
				
				//~ 通知内容
				$m_headline = sprintf( __('恭喜，你已成功兑换<a href="%1$s" target="_blank">%2$s</a>','momo'), get_permalink($pid), $gift_title );
				//~ 发邮件通知
				if(is_email($current_user->dmeng_verify_email)){
					$m_content = '<h3>'.sprintf( __('%1$s，你好！','momo'), $current_user->display_name ).'</h3><p>'.$m_headline.'<br/>'.sprintf( __('温馨提示：%s','momo'), get_option('dmeng_gift_tips', __('兑换成功后请留意信息通知，如有兑换后可见的内容可直接查看。', 'momo')) ).'</p>';
					dmeng_send_email( $current_user->dmeng_verify_email, strip_tags($m_headline), $m_content );
				}

				//~ 站内信息通知
				add_dmeng_message( $uid, 'unread', current_time('mysql'), __('礼品兑换通知', 'momo'), $m_headline);

				$data['success'] = __('兑换成功！', 'momo');
...
<head>
	...
	<title><?php _e('兑换信息查询', 'momo'); ?></title>
	<style type="text/css">
		html {
			background: #f1f1f1;
		}
...
<?php
		...
		$tips = array(
			'text' => __('请输入唯一识别码进行查询，格式如：1-62-1417634028-62ec07f451', 'momo'),
			'color' => '#444'
		);
	...
	if ( wp_verify_nonce( trim($_POST['_wp_nonce']), 'check-nonce' ) ){
		...
		if( count($explode)==4 ){
			...
			if( $code == dmeng_get_exchange_key( $explode[0], $explode[1], $exdate ) ){
				if(dmeng_meta_exists($explode[0], 'exchange_'.$explode[1], $exdate)){
						...
						if( $gift_info ){
							...
							$tips = array(
								...
								$data[] = __('兑换人：', 'momo') . '（<a href="'.get_author_posts_url( $explode[0] ).'" target="_blank">' .__('个人主页', 'momo').'</a>）' . 
													( is_email($user->dmeng_verify_email) ? $user->dmeng_verify_email : __('没有验证邮箱', 'momo') );
								
								$data[] = __('兑换时间：', 'momo') . $exdate;
								
								$data[] = __('礼品标题：', 'momo') . '<a href="'.get_permalink( $explode[1] ).'" target="_blank">' . esc_html(get_the_title($explode[1])).'</a>';
									
								$data[] = __('市场价格：', 'momo') . sprintf("%.2f", intval($gift_info->price));
								
								$credit = intval($gift_info->credit)==0 ? __('免费', 'momo') : intval($gift_info->credit);
								$data[] = __('所需积分：', 'momo') . $credit;
								
								$express = dmeng_get_express_array();
								$data[] = __('物流配送：', 'momo') . $express[intval($gift_info->express)];
							}
						}else{
							$tips = array(
								'text' => __('错误：没有找到对应的礼品信息', 'momo'),
								'color' => '#a94442'
								);
						}
				...
				$tips = array(
					'text' => __('错误：没有找到对应的兑换信息', 'momo'),
					'color' => '#a94442'
					);
				}
			}else{
				$tips = array(
					'text' => __('错误：唯一识别码没有通过验证，你确定这正确吗？', 'momo'),
					'color' => '#a94442'
					);
			}
		}
...
?>
	...
	<form action="<?php echo $check_url;?>" method="POST">
		...
		<button type="submit" class="button"><?php _e('查询', 'momo');?></button>
		<a href="<?php echo $check_url;?>" class="button"><?php _e('刷新本页', 'momo');?></a>
		...
		?>
		<p><?php _e('本功能主要用于校验兑换礼品用户身份，每个识别码都是唯一的，私密的。', 'momo');?></p>
		<p><a href="<?php echo home_url();?>"><?php echo __('&laquo; 返回', 'momo').get_bloginfo('name');?></a></p>
