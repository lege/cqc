...
function dmeng_redirect_wp_admin(){
	if( is_admin() && is_user_logged_in() && !current_user_can( apply_filters('dmeng_admin_base_cap', 'edit_users') ) && ( !defined('DOING_AJAX') || !DOING_AJAX )  ){
...
function dmeng_edit_post_link($url, $post_id){
	if( !current_user_can( apply_filters('dmeng_admin_base_cap', 'edit_users') ) ){
...
function dmeng_user_profile_widget(){
	...
	if(!is_email($current_user->user_email)){
		...
		$li_output .= '<li><a href="'.dmeng_get_user_url('profile').'#pass">'.__('【重要】请添加正确的邮箱以保证账户安全','momo').'</a></li>';
		
	}
	
	if( $current_user->user_email != $current_user->dmeng_verify_email ){
		
		$li_output .= '<li><a href="'.dmeng_get_user_url('profile').'#pass">'.__('【重要】请验证一个邮箱用以接收通知','momo').'</a></li>';
	...
	<li>
		<div class="input-group">
			<span class="input-group-addon">'.__('本页推广链接','momo').'</span>
