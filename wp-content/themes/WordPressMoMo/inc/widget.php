...
function dmeng_in_widget_form($widget, $return, $instance ){
	...
	?>  
<p>
	<button type="button" class="button widget_show_option_toggle"><?php _e('设置显示位置/终端', 'momo');?></button> 
	<button type="button" class="button widget_show_option_checked" style="display:none"><?php _e('全选', 'momo');?></button> 
	<button type="button" class="button widget_show_option_clear" style="display:none"><?php _e('全不选', 'momo');?></button> 
</p>
...
<div style="display:none">
	...
	<p><?php _e('显示位置', 'momo');?></p>

	<hr />

	<p><?php _e('勾上代表在该项对应页面显示，后面输入框输入排除的ID，以逗号隔开，如果只输入ID而不勾选该项则代表只在这些ID页面显示。', 'momo');?></p>
	...
	<p> <?php _e('显示终端', 'momo');?> &nbsp; 
		...
		<label for="<?php echo $widget->get_field_id( $pc_key ); ?>"><?php _e('PC', 'momo');?></label> &nbsp; 
		...
		<label for="<?php echo $widget->get_field_id( $mb_key ); ?>"><?php _e('Moblie', 'momo');?></label> &nbsp; 
...
class DmengAnalyticsWidget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'analytics', __( ' 站点统计' , 'momo' ) , array('classname' => 'widget_analytics', 'description' => __( '站点统计信息' , 'momo' ) ) );
	...
	function widget( $args, $instance ) {
		...
		$list = array(
			...
			'post' => array( __('文章','momo'), wp_count_posts('post')->publish ),



			'cat' => array( __('分类','momo'), wp_count_terms('category') ),



			'tag' => array( __('标签','momo'), wp_count_terms('post_tag') ),



			'user' => array( __('用户','momo'), $users ),



			'comment' => array( __('评论','momo'), wp_count_comments()->total_comments ),



			'view' => array( __('浏览总数','momo'), get_dmeng_traffic_all() ),



			'search' => array( __('搜索次数','momo'), get_dmeng_traffic_all('search') )
		...
		$output .= '<li class="update">'.__('统计时间', 'momo') . ' : <time title="'.sprintf(__('统计数据最后更新时间：%s', 'momo') , current_time('mysql') ).' ">'.date( 'm-d H:i', current_time( 'timestamp', 0 ) ).'</time></li>';
...
?>
	...
	<p>
		...
		<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('标题：','momo');?></label>
...
class DmengOpenWidget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'open_login', __( '登录和资料' , 'momo' ) , array('classname' => 'widget_open_login', 'description' => __( '用户登录&个人资料展示' , 'momo' ) ) );
	...
	if( is_user_logged_in() ){ 
		...
		if( $instance['qq'] && dmeng_is_open_qq() ) {
			...
			$output .= '<li class="icon qq"><a href="'.dmeng_get_open_login_url('qq', 'login', dmeng_get_current_page_url()).'" rel="nofollow" data-no-instant>'.__( '使用QQ账号登录' , 'momo' ).'</a></li>';
		...
		if( $instance['weibo'] && dmeng_is_open_weibo() ) {
			...
			$output .= '<li class="icon weibo"><a href="'.dmeng_get_open_login_url('weibo', 'login', dmeng_get_current_page_url()).'" rel="nofollow" data-no-instant>'.__( '使用微博账号登录' , 'momo' ).'</a></li>';
		...
		if( $instance['wordpress'] ) {
			...
			$output .= '<li class="icon wordpress"><a href="'.wp_login_url(dmeng_get_current_page_url()).'" rel="nofollow" data-no-instant>'.__( '使用本地账号登录' , 'momo' ).'</a></li>';
...
<p>
	...
	$list = array(
		...
		'qq' => __('显示QQ登录（如果有启用QQ登录）','momo'),



		'weibo' => __('显示微博登录（如果有启用微博登录）','momo'),



		'wordpress' => __('显示本地登录','momo'),
...
class DmengCreditRankWidget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'creditRank', __( ' 积分排行榜' , 'momo' ) , array('classname' => 'widget_credit_rank', 'description' => __( '用户可用积分排行榜' , 'momo' ) ) );
...
?>
	...
	<p>
		...
		<label for="<?php echo $this->get_field_id('exclude'); ?>"><?php _e('除了：','momo');?></label>
		...
		<small><?php _e('用户ID，多个ID请用英文逗号（,）隔开','momo');?></small>
...
class DmengRankWidget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'dmengRank', __( ' 排行榜' , 'momo' ) , array('classname' => 'widget_dmeng_rank', 'description' => __( '热门文章/热议文章/搜索排行榜' , 'momo' ) ) );
		...
		if($search_rank){
			...
			foreach( $search_rank as $search ){
				...
				$data[] = array(
					...
					'badge_title' => sprintf(__('%s次搜索','momo'),$search->traffic)
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$data[] = array(
					...
					'badge_title' => sprintf(__('%s条评论','momo'),get_comments_number())
		...
		if($vote_rank){
			...
			foreach( $vote_rank as $vote ){
				...
				$data[] = array(
					...
					'badge_title' => sprintf(__('%s人投票','momo'),$vote->count)
		...
		if($view_rank){
			...
			foreach( $view_rank as $view ){
				...
				$data[] = array(
					...
					'badge_title' => sprintf(__('%s次浏览','momo'),$view->traffic)
...
<p>
	...
	$list = array(
		...
		'search' => __('搜索次数最多的关键词','momo'),



		'comment' => __('评论最多的内容','momo'),



		'vote' => __('最多人投票的内容','momo'),



		'view' => __('浏览次数最多的内容','momo'),
	...
	<p>
		...
		<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('显示数量：','momo');?></label>
...
class DmengRecentUserWidget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'recent_user', __( ' 最近登录用户' , 'momo' ) , array('classname' => 'widget_recent_user', 'description' => __( '显示最近登录用户头像' , 'momo' ) ) );
...
?>
		...
		<ol class="carousel-indicators">
                ...
                <?php if (have_posts()) :query_posts("showposts=1&cat=$cat ");  ?>

                <?php while (have_posts()) : the_post(); 

				$thumbnail = dmeng_get_the_thumbnail();
				$thummore =  get_bloginfo('template_url')."/timthumb.php?src=$thumbnail&h=200&w=360";
	
				$thumbnail_html =  '<img src="'.$thummore.'" alt="'.get_the_title().'">';
					
				?>	
                <div class="item active">
			<a href="<?php echo get_permalink(); ?>"><? echo $thumbnail_html ?></a>
			<div class="carousel-caption"><? the_title() ?></div>
		</div>
		...
		<?php if (have_posts()) :query_posts("cat=$cat &offset=1");  ?>
		...
		<?php endif; wp_reset_query();?>
            ...
            <a class="carousel-control left" data-slide="prev" href="#myCarousel" role="button"><span class="glyphicon glyphicon-chevron-left"></span></a>

            <a class="carousel-control right" data-slide="next" href="#myCarousel" role="button"><span class="glyphicon glyphicon-chevron-right"></span></a>
...
<?
	...
	function form( $instance ) {
		...
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
        	...
        	<label for="<?php echo $this->get_field_id( 'depth' ); ?>">请填写分类目录的id</label>
