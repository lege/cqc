...
function dmeng_admin_notices_action() {
	global $pagenow;
	$message = __('MoMo主题提示 : ','momo');
	if ( 'options-discussion.php' == $pagenow ){
		if(isset( $_GET['settings-updated'] )){
			update_option('thread_comments',1);
		}
		$message .= __('评论嵌套是必须选择的，无法改变！','momo');
		$type = 'error';
	}
...
function dmeng_nav_menus_help_tab(){
    ...
    $screen->add_help_tab( array(
        ...
        'title'	=> __('图标和按钮效果', 'momo'),
        'content'	=> '<p>' . __( '请您留意：菜单项图标和按钮效果属于MoMo主题专属功能，这代表着当你切换到其他主题，图表和按钮效果将不复存在。', 'momo' ) . '</p>'
								. '<p>' . __( '首先，请您缩小“帮助”选项卡，并打开“显示选项”在“显示菜单高级属性”中勾上“CSS类”项，然后在编辑菜单项时填入以下内容：', 'momo' ) . '</p>'
								. '<p>' . __( '添加图标：在允许 Bootstrap 免费使用的200个 <a href="http://cdn.dmeng.net/glyphicons.html" target="_blank">Glyphicons 字体图标</a> 中挑选出你喜欢的图标，复制图标类名到”CSS类“里的第一个即可（必须是在前面第一个），如相机图标则是 <span style="color: #339966;">glyphicon-camera</span> ，以此类推。', 'momo' ) . '</p>'
								. '<p>' . __( '添加按钮：根据 Bootstrap 的按钮CSS类名填写到”CSS类“中即可，如：', 'momo' ) . '</p>'
								...
								. '<p>' . __( '如果既需要前置图标也需要按钮效果，请这样填写”CSS类“：<span style="color: #339966;">glyphicon-camera btn btn-defaul</span> （图标CSS类名在第一个）', 'momo' ) . '</p>'
								. '<p>' . __( '更详细的使用教程请看：<a href="http://www.dmeng.net/wordpress-navigation-menus.html" target="_blank">WordPress导航菜单设置</a>', 'momo' ) . '</p>'
    ) );
