...
<div class="pinglun" id="respond">
  <?php if(get_option('comment_registration')&&!$user_ID ): ?>
  <p><?php printf(__('需要 <a href="%s">登录</a> 才能发表评论'),get_option('siteurl')."/wp-login.php?redirect_to=".urlencode(get_permalink())); ?></p>
  <?php else: ?>
  <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" name="commentform">
    ...
    <input type="submit" class="submit" id="submit" tabindex="5" value="<?php echo attribute_escape(__('发表评论')); ?>" />
